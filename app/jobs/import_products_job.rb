class ImportProductsJob < ApplicationJob
  ...
  def perform(product_imports)
    user = User.find(product_imports.created_by)
    # user = product_imports.user
    begin
      SolidusImportProducts::Import.call(product_imports: product_imports)
      UserMailer.product_import_results(user).deliver_later
      ...
      UserMailer.product_import_results(user, "#{exception.message}  #{exception.backtrace.join('\n')}").deliver_later
    end
  end
end
